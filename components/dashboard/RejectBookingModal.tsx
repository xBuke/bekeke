  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`/api/bookings/${booking.id}/reject`, {
        ...
        body: JSON.stringify({
          provider_notes: rejectionReason.trim() || null
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Greška pri odbijanju zahtjeva');
      }

      toast.success('Zahtjev odbijen');
      onSuccess();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Greška pri odbijanju zahtjeva');
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-2xl mx-4 sm:mx-0">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <Textarea
              id="rejection-reason"
              placeholder="Objasnite zašto odbijate zahtjev..."
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button 
              ...
              variant="destructive"
            >
              {loading ? 'Odbija se...' : 'Odbij zahtjev'}
            </Button>
          </div>
        </div>
      </DialogContent>
